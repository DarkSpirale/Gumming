...
namespace DTerrain
{
    ...
    public class PaintableChunk : MonoBehaviour, ITexturedChunk
    {
        ...
        public virtual bool Paint(RectInt r, PaintingParameters pp, Color[] rc)
        {
            ...
            else if (pp.PaintingMode == PaintingMode.FROM_TEXTURE)
            {
                if (common.Equals(r) || common.y <= r.y)
                {
                    cs = rc;
                }
                else
                {
                    int shiftValue = r.height - common.height;
                    for (int i = 0; i < common.height; i++)
                    {
                        cs[i] = rc[i + shiftValue];
                    }
                }
            }
